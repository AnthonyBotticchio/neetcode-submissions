...
#define DELIMETER '#'
   ...
   public:
    ...
    vector<string> decode(string s) {
        ...
        int idx{};
        while (idx < s.size()) {
            auto delim_itr = std::find(
                s.begin() + idx, s.end(),
                DELIMETER);  // find offers a place to start searching from. We start from idx
            if (delim_itr == std::end(s)) break;

            auto delim_idx = std::distance(s.begin(), delim_itr);
